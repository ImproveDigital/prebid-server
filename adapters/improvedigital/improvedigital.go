...
func getBidType(bid openrtb2.Bid, imps []openrtb2.Imp) (openrtb_ext.BidType, error) {
	...
	if isMultiFormatImp(imp) && bid.MType == 0 {
		return "", &errortypes.BadServerResponse{
			Message: fmt.Sprintf("Bid must have non-zero MType for multi format impression with ID: \"%s\"", bid.ImpID),
		}
	}

	// check for bid w.r.t multi format imp first, for which MType will be non-zero
	if bid.MType != 0 {
		switch bid.MType {
	...
	// Not a bid against multi format imp and MType is zero. So, fallback to imp based detection logic
	if imp.Banner != nil {
		return openrtb_ext.BidTypeBanner, nil
	...
	return "", &errortypes.BadServerResponse{
		Message: fmt.Sprintf("Could not determine bid type for impression with ID: \"%s\"", bid.ImpID),
	}
}
